...
public class Pong extends GameBase {
	...
	protected final String[] actions = { "UP / W -> Moves paddle up (or previous option on menu)",
			"DOWN / S -> Moves paddle down (or next option on menu)", "Space -> Pauses game until pressed again.",
	...
	public void updateBot() {
		if (botCooldown > 0)
			...
			if (bot.y + bot.height / 2 + bot.speed < ball.y)
				bot.move(false);
			else if (bot.y + bot.height / 2 + bot.speed > ball.y + 5)
				bot.move(true);
			botCooldown = 1;
		}
	}
	...
	public void keyPressed(KeyEvent ev) {
		...
		if (ingame) {
			...
			if (id == KeyEvent.VK_SPACE)
				paused = paused || countdown > 0 ? false : true;
