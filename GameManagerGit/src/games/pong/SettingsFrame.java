...
public class SettingsFrame extends BaseFrame {
	...
	public SettingsFrame(Pong pong) {
		...
		int[][] bounds = { { 10, 11, 278, 70, 345, 11, 75, 49, 298, 11, 37, 49, 430, 11, 47, 49 },
				{ 10, 92, 320, 70, 375, 92, 52, 49, 335, 92, 40, 49, 430, 92, 47, 49 } };
	...
	public void setupLabels(JLabel lab1, JLabel lab2, JLabel lab3, JLabel lab4, boolean score, int[] bnds) {
		lab1.setIcon(new ImageIcon(score ? scoreImg : ballImg));
		lab2.setIcon(getImage(get(score)));
		lab3.setIcon(new ImageIcon(subt));
		lab4.setIcon(new ImageIcon(plus));
		lab1.setBounds(bnds[0], bnds[1], bnds[2], bnds[3]);
		lab2.setBounds(bnds[4], bnds[5], bnds[6], bnds[7]);
		lab3.setBounds(bnds[8], bnds[9], bnds[10], bnds[11]);
		lab4.setBounds(bnds[12], bnds[13], bnds[14], bnds[15]);
		getContentPane().add(lab1);
		getContentPane().add(lab2);
		getContentPane().add(lab3);
		getContentPane().add(lab4);
		lab3.addMouseListener(new ActionReader() {
			@Override
		...
		lab4.addMouseListener(new ActionReader() {
			@Override
			public void mouseClicked(MouseEvent arg0) {
	...
	public int get(boolean score) {
		if (score)
			return pong.maxScore;
		else
			return pong.gameSpeed;
	}
