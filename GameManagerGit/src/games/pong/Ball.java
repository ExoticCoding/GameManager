...
public class Ball {
	...
	public int checkCollision(Paddle paddle) {
		if (x < paddle.x + paddle.width && x + width > paddle.x && y < paddle.y + paddle.height
				&& y + height > paddle.y)
			return 1;
		else if ((paddle.x > x && paddle.player) || (paddle.x < x - width && !(paddle.player)))
			return 2;
		else
			return 0;
	}
